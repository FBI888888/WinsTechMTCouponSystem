...
function AuthPage({ onAuthSuccess }) {
  ...
  useEffect(() => {
    if (status && status.message) {
      const t = status.type === 'success' ? 'success' : status.type === 'error' ? 'error' : 'info'
      showToast(t, status.message)
    }
  }, [status])
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">美团联盟个人版查询返利工具</p>
        </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-medium ${licenseInfo.expiresAt ? 'text-orange-600' : 'text-green-600'}`}>
                  {licenseInfo.expiresAt ? new Date(licenseInfo.expiresAt).toLocaleDateString('zh-CN') : '永久有效'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-600 flex items-center gap-1">
