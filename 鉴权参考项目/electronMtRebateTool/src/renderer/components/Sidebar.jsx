...
import { Users, BadgeDollarSign, Gift } from 'lucide-react'

const menuItems = [
  { id: 'account', label: '账号管理', icon: Users, description: '账号管理' },
  { id: 'rebate', label: '返利查询', icon: BadgeDollarSign, description: '查询订单返利' }
]
    ...
    <aside className="w-48 bg-gradient-to-b from-slate-900 to-slate-800 text-white flex flex-col shadow-xl">
      <div className="p-4 border-b border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-lg">返利查询工具</h1>
            <p className="text-xs text-slate-300">v1.3.0</p>
          </div>
        </div>
      </div>
              ...
              <li key={item.id}>
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-2 py-3 rounded-xl transition-all duration-200 group
                    ${isActive ? 'bg-white text-slate-900 shadow-lg' : 'text-slate-200 hover:bg-white/10 hover:text-white'}`}
                >
                  <Icon className={`w-5 h-5 transition-transform ${isActive ? 'scale-110' : 'group-hover:scale-110'}`} />
                  <div className="text-left">
                    <div className="font-medium text-sm">{item.label}</div>
                    <div className={`text-xs ${isActive ? 'text-slate-500' : 'text-slate-400'}`}>{item.description}</div>
                  </div>
                </button>
              </li>
            )
