...
class MeituanAPI {
   ...
   * 获取订单列表（带状态筛选）
   */
  static async getOrdersListWithStatus(userid, token, days = 7, statusFilter = 0) {
    const allOrders = []
    let offset = 0
    let check = true
    ...
    while (check) {
      ...
      try {
        // 使用自定义 transformResponse 来保持大整数为字符串
        // 将 orderid 等大数字字段转换为字符串，避免精度丢失
        const response = await axios.get(url, {
          headers,
          timeout: 15000,
          transformResponse: [(data) => {
            // 将 JSON 中的大整数转换为字符串（匹配 "orderid": 后面的数字）
        ...
        for (const order of orders) {
          ...
          if (orderTime >= daysAgo) {
            ...
            // 调试日志：检查 orderid 类型和值
            if (allOrders.length === 0) {
              console.log('订单解析调试 - 原始orderid类型:', typeof order.orderid, '值:', order.orderid)
              console.log('订单解析调试 - stringOrderId:', order.stringOrderId)
              console.log('订单解析调试 - 最终使用:', orderIdStr)
            }
  ...
  static async getCouponListByOrderId(token, orderid, options = {}) {
    ...
    console.log('getCouponListByOrderId - orderid:', orderIdStr, '是否礼物订单:', isGift, '经度:', longitude, '纬度:', latitude)

    if (isGift) {
      return await this.getGiftCouponList(token, orderIdStr, { longitude, latitude })
    }
    ...
    for (let retry = 0; retry < maxRetries; retry++) {
      try {
        ...
        console.log('获取券码 - 响应数据:', JSON.stringify(response.data).substring(0, 500))
  ...
  static async getGiftCouponList(token, giftId, options = {}) {
    ...
    // 构建 location 对象，如果提供了经纬度则使用，否则只有 accuracy
    // 兼容不同接口可能使用的字段：longitude/latitude 或 lng/lat
    const locationObj = { accuracy: 0 }
    if (longitude) {
  ...
  static parseCouponResponse(res) {
    ...
    try {
      ...
      try {
        const navBar = res?.data?.nodeDataMap?.OrderDetailNavBar1?.props
        title = navBar?.bizParams?.extra?.poiName?.split('（')[0] || navBar?.bizParams?.extra?.poiName || ''
      ...
      // 解析未核销券码
      try {
        const coupons = res?.data?.nodeDataMap?.OrderDetailFoodNps1?.props?.couponInfo?.coupons || []
        for (const coupon of coupons) {
          if (coupon.useStatus === 1) {
            couponsInfoList.push({
              title,
              coupon: (coupon.code || '').replace('优惠码:', ''),
              status: `${coupon.code}--未核销`,
              order_status: coupon.statusText || '',
              mobile,
              payPrice
            })
          }
      ...
      // 解析已核销券码
      try {
        const verifyRecords = res?.data?.nodeDataMap?.OrderDetailFoodNps1?.props?.couponInfo?.verifyRecords || []
        for (const record of verifyRecords) {
          ...
          for (const coupon of (record.verifyCoupons || [])) {
            couponsInfoList.push({
              title,
              coupon: coupon.replace('优惠码:', ''),
              status: `${coupon}--核销时间：${verifyTime}--核销门店："${poiName}"`,
              mobile,
              order_status: verifyStatusText,
              payPrice
            })
          }
        }
  ...
  static parseGiftCouponResponse(res) {
    ...
    try {
      // 解析未核销券码
      try {
        const coupons = res?.data?.nodeDataMap?.CouponModule1?.props?.coupons || []
        for (const coupon of coupons) {
          if (coupon.useStatus === 1) {
            couponsInfoList.push({
              title: '',
      ...
      // 解析已核销券码
      try {
        const verifyRecords = res?.data?.nodeDataMap?.FoodOrderDetailVerifyRecord1?.props?.verifyRecords || []
        for (const record of verifyRecords) {
          const verifyTime = record.verifyTime ? new Date(record.verifyTime * 1000).toLocaleString('zh-CN') : ''
          const poiName = record.poiName || ''
          const verifyStatusText = record.verifyStatusText || ''
